...
def process_COMMAND_BL_GET_VER(length_to_follow):
    ver=read_serial_port(length_to_follow)
    value = bytearray(ver)
    print("\n   Bootloader Version:", ver.decode( 'utf-8' ), " \n"  )
...
ports = glob.glob('/dev/tty.*')
print( "\n   Available Serial Ports on your PC :")
for i in range(len(ports)):
    print("   ",i, " : ", ports[i])
print("\n   Select the port number from above list")
port_num = int(input("\n   Enter the port number here :"))
ret=Serial_Port_Configuration(ports[port_num])
if(ret < 0):
